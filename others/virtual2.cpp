...
int main()
{
    ...
    cout << "dev_class 的 第二个函数首地址：" << (int*)*(int*)&dc+2 << endl;
    cout << "dev_class 的 第三个函数首地址：" << (int*)*(int*)&dc+4 << endl;
    cout << "dev_class 的 第四个函数首地址：" << (int*)*(int*)&dc+6 << endl;
    cout << "dev_class 的 第五个函数首地址：" << (int*)*(int*)&dc+8 << endl;
    cout << "dev_class 的虚函数表结束标志: " << *((int*)*(int*)&dc+10) << endl;
    // 通过函数指针调用函数，验证正确性
    typedef void(*func_pointer)(void);
    func_pointer fp = NULL;
    for (int i=0; i<5; i++) {
        fp = (func_pointer)*((int*)*(int*)&dc+2*i);
        fp();
    }
    return 0;
}
